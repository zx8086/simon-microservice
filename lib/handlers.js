...
exports.health = async (request, h) => {
  try {
    ...
    return h.response(result).code(200).type('application/json').code(200)
  } catch (error) {
  ...
  try {
    ...
    try {
      ...
      if (!scopeExists) {
        ...
        return h.response({ message: `Scope '${scopeName}' does not exist` }).code(404)
      }
      ...
      if (!collectionExists) {
        ...
        return h.response({ message: `Collection '${collectionName}' does not exist in scope '${scopeName}'` }).code(404)
      }
      ...
      return h.response({ message: 'Index created successfully' }).code(200);
    } catch (error) {
      console.error(`Error creating primary index for '${collectionName}' collection in scope '${scopeName}':`, error)
      return h.response({ message: 'Failed to create index', error: error.message }).code(500)
    }
  } catch (error) {
    console.error('Failed to connect to Couchbase:', error)
    logAndWriteToLogger(`Failed to connect to Couchbase: ${error.message}`)
